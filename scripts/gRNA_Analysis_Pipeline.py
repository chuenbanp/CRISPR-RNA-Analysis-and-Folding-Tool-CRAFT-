...
def generate_colored_plots(rnafold_output, output_dir, id_suffix=""):
    """
...
def generate_excel_report(gRNA_data, output_dir):
    ...
    with pd.ExcelWriter(report_path, engine='xlsxwriter') as writer:
        ...
        for idx, grna in enumerate(gRNA_data):
            ...
            img_path_orig = os.path.join(output_dir, f"{grna['id']}_original_ss.png")
            if os.path.exists(img_path_orig):
                worksheet.insert_image(row_num, 7, img_path_orig, image_options)

            # Insert adjusted image
            img_path_adj = os.path.join(output_dir, f"{grna['id']}_ss.png")
...
def main():
    ...
    excel_file_path = input("Please drag and drop your Excel file here and press Enter: ").strip().replace("'", "")
    ...
    if not gRNA_data:
        ...
        return
    # Name output folder after input file (without extension)
    input_base = os.path.splitext(os.path.basename(excel_file_path))[0]
    output_dir = os.path.join(os.path.dirname(excel_file_path), f"gRNA_structures_{input_base}")
    os.makedirs(output_dir, exist_ok=True)
    print(f"\n✅ Found {len(gRNA_data)} valid gRNAs. Saving results in: {output_dir}")
    ...
    try:
        ...
        completed_process_adjusted = subprocess.run(command, input=fasta_content_adjusted.encode('utf-8'), check=True, cwd=output_dir, capture_output=True)
    ...
    try:
        command = ["RNAfold", "-p", "-T", "37"]
        completed_process_original = subprocess.run(command, input=fasta_content_original.encode('utf-8'), check=True, cwd=output_dir, capture_output=True)
    ...
    generate_colored_plots(rnafold_output_adjusted, output_dir)
    generate_colored_plots(rnafold_output_original, output_dir, id_suffix="_original")
    ...
    if not gs_command:
        ...
        return
    for grna in gRNA_data:
        ps_file_adjusted = os.path.join(output_dir, f"{grna['id']}_ss.ps")
        png_file_adjusted = os.path.join(output_dir, f"{grna['id']}_ss.png")
        if os.path.exists(ps_file_adjusted):
            try:
                ...
                if os.path.exists(png_file_adjusted):
                    ...
                    img = img.resize((781, 515), resample_mode)
        ...
        ps_file_original = os.path.join(output_dir, f"{grna['id']}_original_ss.ps")
        png_file_original = os.path.join(output_dir, f"{grna['id']}_original_ss.png")
